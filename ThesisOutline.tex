...
\begin{thm}
    All submartingales have an right continuous with left limit (RCLL) modification
\end{thm}
...
We are given a filtered probability space $\cFPS$ s.t. $\cF_0 \subset \mathcal{W}$ \\
...
Specifically, we define $A := \{\E[Y_\tau - Y_\theta \quantifyspace \cF_{\tau \vee \theta}] \geq 0\}$ and then define $\gamma := \tau 1_A + \theta 1_{A^c}$ \\
...
\end{defn}
\cheekynewline
We also impose the condition that $\E[\sup_{t \in [0,T]} Y_t] < \infty$, using this condition we can achieve sub-objective (ii). \\
