...
\begin{thm}
    ...
    Since $X$ is a semimartingale and $H$ has an absolutely continuous derivative so we can apply the multi dimensional Itô-Tanaka-Meyer formula to $f$ to find,
    \[
        f(t) = f(0) + \int_0^t -r f(u) \df{u} + \int_0^t f_x(u) \big (r X_u \df{u} + \sigma X_u \df{W_u} \big ) +
